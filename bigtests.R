...
ii <- import.big.data(in.fn); b1 <- getBigMat(ii) # SLOWER without dimnames!
#2. import using row/col names from file
ii <- import.big.data(in.fn,
 cols.fn="colnames.txt",rows.fn="rownames.txt"); b2 <- getBigMat(ii)
#3. import by passing colnames/rownames as objects
ii <- import.big.data(in.fn,
 col.names=coln,row.names=rown); b3 <- getBigMat(ii)
...
t5 <- test_that("RowNamesWrongWarn", expect_warning(import.big.data(in.fn,
                              col.names=coln,row.names=rown)) )
ii <- import.big.data(in.fn,col.names=coln,row.names=rown); b3 <- getBigMat(ii)
...
ii <- import.big.data(infs,
 cols.fn="colnames.txt",rows.fn=rowfs); b2 <- getBigMat(ii)
ii <- import.big.data(infs,
 col.names=coln,row.names=rownL); b3 <- getBigMat(ii)
# check all last 2 against expected result
...
ii <- import.big.data(infs,
 cols.fn=colfs,rows.fn="rownames.txt"); b2 <- getBigMat(ii)
ii <- import.big.data(infs,
 col.names=colnL,row.names=rown); b3 <- getBigMat(ii)
# check all last 2 against expected result
